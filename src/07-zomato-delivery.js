...
export function confirmOrder(order) {
  // Your code here
  return new Promise((resolve, reject) => {
    if (!order || typeof order !== "object" || !order.orderId || order.status !== "placed") {
...
export function assignRider(order) {
  ...
  return new Promise((resolve, reject) => {
    if (!order || typeof order !== "object" || order.status !== "confirmed") {
...
export function deliverOrder(order) {
  // Your code here
  return new Promise((resolve, reject) => {
    if (!order || typeof order !== "object" || !order.orderId || order.status !== "assigned" || !order.rider) {
      reject(new Error("No rider assigned!"));
    }
    resolve({ ...order, status: "delivered", deliveredAt: new Date().toISOString() });
  });
}
...
export function processMultipleOrders(orderList) {
  // Your code here
  const promises = orderList.map(order => processDelivery(order.restaurant, order.items));
