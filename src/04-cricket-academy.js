...
export class Batsman extends Player {
  ...
  getStrikeRate() {
    ...
    const totalStrikeRate = this.innings.reduce((acc, innings) => acc + innings.strikeRate, 0)
  ...
  getProfile() {
    ...
    const totalRuns = this.innings.reduce((acc, innings) => acc + innings.runs, 0)
...
export class Bowler extends Player {
  ...
  getEconomy() {
    ...
    const totalEconomy = this.spells.reduce((acc, spell) => acc + parseFloat(spell.economy), 0)
  ...
  getProfile() {
    // Your code here
    const baseProfile = super.getProfile()
    const totalWickets = this.spells.reduce((acc, spell) => acc + spell.wickets, 0)
...
export class AllRounder extends Player {
  ...
  getProfile() {
   ...
   return {
      ...
      bowlingStyle: this.bowlingStyle,
      role: "allrounder",
      totalRuns: this.innings.reduce((acc, innings) => acc + innings.runs, 0),
      totalWickets: this.spells.reduce((acc, spell) => acc + spell.wickets, 0),
      inningsPlayed: this.innings.length,
      spellsBowled: this.spells.length
    }
   }
