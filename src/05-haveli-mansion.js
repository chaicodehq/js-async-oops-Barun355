...
export class HaveliSecurity {
  ...
  verifyAccess(name) {
    // Your code here
    const isAllowed = this.isResident(name);
    this.#accessLog.push({ name, time: new Date().toISOString(), allowed: isAllowed });
    if (isAllowed) {
  ...
  getAccessLog(passcode) {
    ...
    return [...this.#accessLog]; // Return a copy of the access log
  }
